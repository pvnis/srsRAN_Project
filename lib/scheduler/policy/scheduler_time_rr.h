    ...
    for (auto& ue : ues_slice) {
      for (unsigned i = 0; i != ue->nof_cells(); ++i) {
        ...
        if (candidates.begin() != candidates.end()){
          ...
          mcs = ue_cc.required_dl_prbs(pdsch, ue->pending_dl_newtx_bytes(), dci_type);
          logger.debug("Poll: UE {} in slice sst={} sd={} needs {} PRBs", ue_cc.rnti(), s_nssai.sst, s_nssai.sd, mcs.n_prbs);
          prbs += mcs.n_prbs;
          // Update mcs description
          ue->mcs_description = mcs.mcs_description;
        }
